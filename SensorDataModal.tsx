...
interface SensorDataModalProps {
  ...
  sensorData: {
    ...
    name: string;
    moisture: number;
    status: string; // Already translated when set in App.tsx
  } | null;
}
...
export const SensorDataModal: React.FC<SensorDataModalProps> = ({ isOpen, onClose, sensorData }) => {
  ...
  // Assuming sensorData.status is already translated. We just pick color.
  // The actual text "Optimal", "Dry", "Overwatered" comes from sensorData.status
  // which is set using t() in App.tsx
  if (sensorData.status === t('sensorModal.status.dry')) {
    statusColor = 'text-yellow-600 dark:text-yellow-400';
  } else if (sensorData.status === t('sensorModal.status.overwatered')) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 className="text-xl font-semibold">{t('sensorModal.title', { sensorName: sensorData.name })}</h3>
          <button onClick={onClose} className="text-primary hover:text-yellow-300" aria-label={t('sensorModal.closeButtonAriaLabel', {defaultValue: "Close sensor data modal"})}>
        ...
        <div className="space-y-4">
          <div>
            <span className="font-medium text-gray-700 dark:text-gray-300">{t('sensorModal.sensorName.label')}</span>
            <span className="ml-2 text-gray-900 dark:text-gray-100">{sensorData.name}</span>
          </div>
          <div>
            <span className="font-medium text-gray-700 dark:text-gray-300">{t('sensorModal.moistureLevel.label')}</span>
            <span className="ml-2 text-2xl font-bold text-blue-600 dark:text-blue-400">{sensorData.moisture}%</span>
          </div>
          <div>
            <span className="font-medium text-gray-700 dark:text-gray-300">{t('sensorModal.status.label')}</span>
