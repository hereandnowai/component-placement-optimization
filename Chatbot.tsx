...
export const Chatbot: React.FC<ChatbotProps> = ({ chatHistory, onSendMessage, onClose, isLoading }) => {
  ...
  const toggleVoiceInput = useCallback(() => {
    ...
    if (!SpeechRecognitionAPI) {
      alert(t('chatbot.voiceNotSupported', {defaultValue: "Voice recognition is not supported by your browser."}));
      return;
    }
    ...
    recognitionRef.current.lang = t('speechLang', {defaultValue: 'en-US'}); // Use localized language for speech recognition
    ...
    recognitionRef.current.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      alert(t('chatbot.voiceError', {error: event.error, message: event.message || ''}));
      setIsListening(false);
    };
    ...
    <div className="fixed bottom-0 right-0 mb-4 mr-4 md:mb-6 md:mr-6 w-full max-w-md h-[70vh] max-h-[600px] bg-white dark:bg-gray-800 shadow-2xl rounded-lg flex flex-col z-50 transform transition-all duration-300 ease-out">
      <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700 bg-secondary text-primary rounded-t-lg">
        <div className="flex items-center space-x-2">
          <img src={CARAMEL_AVATAR_URL} alt={t('chatbot.avatarAlt', {defaultValue: "Caramel AI Avatar"})} className="w-10 h-10 rounded-full border-2 border-primary" />
          <h3 className="text-lg font-semibold">{t('chatbot.headerTitle')}</h3>
        </div>
        <button onClick={onClose} className="text-primary hover:text-yellow-300" aria-label={t('chatbot.closeAriaLabel', {defaultValue: "Close chat"})}>
      ...
      <div className="p-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-b-lg">
        <div className="flex items-center space-x-2">
          ...
          <Button 
            ...
            variant="icon" 
            aria-label={t(isListening ? 'chatbot.stopListeningAriaLabel' : 'chatbot.startListeningAriaLabel', {defaultValue: isListening ? "Stop listening" : "Start voice input"})} 
            disabled={isLoading} 
            className={`${isListening ? 'bg-red-500 hover:bg-red-600' : 'bg-accent hover:bg-teal-700'} text-white`}
          >
            {isListening ? (
